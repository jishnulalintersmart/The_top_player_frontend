  ...
  return (
    <div className={styles.newsBx}>
      <div className={styles.dElmt}>
        <Image src={"/images/news-dElmt-1.svg"} alt="dElmt" layout="fill" objectFit="contain" />
      </div>
      ...
      <div className={styles.cntWrap}>
        <div className={styles.tle}>{Lang === "ar" ? news?.title_ar : news?.title_en} </div>
        <div className={styles.txt}> {Lang === "ar" ? news?.description_ar : news?.description_en} </div>
        <div className={styles.info}>
          {" "}
          {t("news.postdate")} : {format(new Date(news?.createdAt), "dd MMMM yyyy")}
        </div>
        <Link href={"#news"} className={`${styles.vAllBtn} vAllBtn`} aria-label="view all button">
